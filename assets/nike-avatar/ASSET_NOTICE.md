# AIニケちゃん Asset Notice

AIニケちゃん is included as a bundled CharaDock character with permission.
The character artwork and the derived desktop and RLCD eye and mouth variants are not licensed
under CharaDock's Apache License 2.0.

Required credit:

- AIニケちゃん / AI Nike-chan
- tegnike: https://x.com/tegnike
- Official site: https://nikechan.com/

Permission to bundle these files with CharaDock does not grant permission to
extract, redistribute, sell, train models on, or reuse them as a standalone
asset pack. Obtain separate permission for uses outside CharaDock.
