...
設定から明示的に有効化すると、同じプライベートWi-Fiにあるスマートフォンなどへキャラ中心の画面を表示できます。短時間だけ有効なQRコードで端末ごとにペアリングし、文字によるChat、個別に許可したWork、進捗確認、中断、最近の履歴、成果物カードを利用できます。キャラクターと準備済みの通常TTSモデルをリモート画面から切り替え、PCとスマートフォンの再生を個別にON / OFFできます。PC側でGPT-Liveを開始している間は、リモートの文字入力を同じLiveセッションへ送り、字幕を同期できます。Beatrice 2を設定済みなら変換音声はPC側で再生します。リモート画面はマイク、カメラ、画面撮影、ブラウザ操作、PC操作を要求しません。

ペアリングURLは最初の端末が使用するか10分経過すると無効になります。ペアリング済み端末は設定した有効時間内なら同じURLを再コピーする必要がありません。PC側では端末名、接続状態、最終接続、有効期限を確認し、端末ごとまたは一括で解除できます。PC再起動、IPアドレス変更、有効期限切れ、解除後は再ペアリングが必要です。

v0.1.4ではローカルLAN向けPoCで、通信はHTTPです。初期状態はOFFで、`0.0.0.0`ではなく選択したプライベートIPv4アドレス1つだけで待ち受けます。信頼できる自宅・社内ネットワークでのみ使用し、ルーターのポート開放は行わないでください。高エントロピーの期限付きペアリングトークン、HttpOnly・SameSiteセッションCookie、セッション別CSRF対策、全端末解除、現在の作業先に紐づく成果物だけの制限を実装しています。
