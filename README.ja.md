...
設定から明示的に有効化すると、同じプライベートWi-Fiにあるスマートフォンなどへキャラ中心の画面を表示できます。短時間だけ有効なQRコードで端末ごとにペアリングし、文字によるChat、個別に許可したWork、進捗確認、中断、全画面履歴、成果物カードを利用できます。キャラクターと準備済みの通常TTSモデルをリモート画面から切り替え、PCとスマートフォンの再生を個別にON / OFFできます。GPT-Liveはスマートフォンから開始・停止でき、回答音声と字幕もスマートフォンへ直接届きます。HTTPSで開いた場合はスマートフォンのマイクをそのままLive入力に利用できます。スマートフォン直結LiveではBeatrice 2を経由せず、選択したGPT-Live音声を再生します。
