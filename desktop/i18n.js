...
(function initializeCharaDockI18n(root) {
  ...
  const ENGLISH = Object.freeze({
    ...
    "最新版を確認し、このアプリの入手先から更新します。": "Checks the latest version and updates from where you installed this app.",
    ...
    "この送信で使うMCP接続": "MCP connections for this turn",
    "ファイルまたは拡張を追加": "Add a file or extension",
    ...
    "この送信で使う拡張": "Extensions for this turn",
    "Skills · MCP ／ または @ で検索": "Skills · MCP · Type / or @ to search",
    "SkillまたはMCPを検索": "Search Skills or MCP",
    "Skill・MCPを検索": "Search Skills or MCP",
    "利用できるSkillsとMCP接続": "Available Skills and MCP connections",
    ...
    "MCPの割り当て先": "MCP assignment target",
    "MCPを設定する相手": "MCP target",
    ...
    "Streamable HTTPの接続を保存し、上で選んだ相手のChat・Work・Liveに割り当てます。": "Save Streamable HTTP connections and assign them to the selected target's Chat, Work, and Live sessions.",
    "接続を追加": "Add connection",
    "サーバーを追加": "Add server",
    "保存済み接続": "saved connections",
    "認証なし・APIキーに対応": "No authentication or API key",
    "追加先を信頼できる場合だけ有効にしてください": "Enable only servers you trust",
    "接続先と利用範囲を確認してください": "Review the connection and its scope",
    "割り当てたMCPツールは確認画面を挟まず実行され、情報送信や外部サービスの変更を行う場合があります。信頼できる接続だけを有効にしてください。APIキーは再表示せず、利用できる端末ではOSの暗号化ストレージへ保存します。OAuth 2.0は次のステップで対応します。": "Assigned MCP tools run without another confirmation and may send information or change external services. Enable only connections you trust. API keys are never shown again and are stored in the operating system's secure storage when available. OAuth 2.0 support comes next.",
    "MCPサーバーを追加": "Add MCP server",
    "MCPサーバーを編集": "Edit MCP server",
    "名前とURLだけで追加できます。APIキーが必要な場合は認証方式を切り替えてください。": "A name and URL are enough. Change the authentication method when an API key is required.",
    "表示名": "Display name",
    "例：社内ドキュメント": "Example: Team documentation",
    "認証": "Authentication",
    "認証なし": "No authentication",
    "保存済みのキーは表示しません": "Saved keys are never shown",
    "Authorization: Bearer として送信します。": "Sent as Authorization: Bearer.",
    "APIキーヘッダーの詳細": "API key header details",
    "ヘッダー名": "Header name",
    "接頭辞": "Prefix",
    "不要なら空欄": "Leave blank when unused",
    ...
    "ストリーミング音声認識（日本語・ローカル）": "Streaming speech recognition (Japanese, local)",
    "sherpa-onnx（ローカル）": "sherpa-onnx (local)",
    "端末音声認識": "System speech recognition",
    "OpenAI文字起こし": "OpenAI transcription",
    "ストリーミング認識モデル": "Streaming recognition model",
    "ストリーミング音声モデルを確認しています…": "Checking streaming speech models…",
    "モデルをダウンロード": "Download model",
    "モデルを削除": "Delete model",
    "話している途中から認識結果を表示します。モデルは初回だけ取得し、音声は端末内で処理します。": "Shows interim results while you speak. The model is downloaded once and audio stays on this device.",
    "開始方法": "Activation",
