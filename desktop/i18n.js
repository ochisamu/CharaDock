...
(function initializeCharaDockI18n(root) {
  ...
  const ENGLISH = Object.freeze({
    ...
    "マイクや外出先からの接続に使います。CharaDockは既存のServe設定を上書きせず、ここで開始した設定だけ停止します。": "Use this for microphone input or access away from home. CharaDock never overwrites an existing Serve route and stops only routes it started.",
    ...
    "通常LANは信頼できる自宅・社内Wi-Fiでのみ使用してください。マイクにはHTTPSが必要です。外出先から利用する場合もルーターのポートは開放せず、Tailscale Serveなどアクセス制御された経路を利用してください。": "Use regular LAN access only on trusted home or workplace Wi-Fi. Microphone access requires HTTPS. For remote use, never open the router port; use an access-controlled route such as Tailscale Serve.",
