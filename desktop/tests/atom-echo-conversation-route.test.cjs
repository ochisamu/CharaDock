...
test("ATOM Echo Live uses one hidden WebRTC owner while keeping PC captions in the shared turn", () => {
  const main = fs.readFileSync(path.resolve(__dirname, "..", "main.cjs"), "utf8");
  assert.match(main, /startCodexRealtimeVoice\(payload, "atom-echo"\)/);
  assert.match(main, /target === "atom-echo"[\s\S]{0,500}contents\.send\("chat:stream", message\)/);
  assert.match(main, /atomEchoLiveWindow\.webContents\.send\("atomEcho:realtimeEvent", forwarded\)/);
  assert.match(main, /handleMascotConversation\(message, atomEchoStandardDeliveryOptions\(\)\)/);
