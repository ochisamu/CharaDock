...
test("Realtime sessions start only from voice input, accept typed turns, and keep transcript deltas intact", () => {
  ...
  assert.match(main, /provider: "codex",[\s\S]*streamed: true,[\s\S]*followUp: true/);
  ...
  assert.match(control, /params\.suppressed[\s\S]*setRealtimeOutputSuppressed\(method !== "thread\/realtime\/transcript\/done"\)/);
  assert.match(mascot, /previousProvider === "realtime" && appState\.speechInputProvider !== "realtime"[\s\S]*closeRealtime\(\)/);
  assert.match(mascot, /setRealtimeOutputSuppressed\(suppressed && method !== "thread\/realtime\/transcript\/done"\)[\s\S]*releaseRealtimeBeatriceCaption/);
  ...
  assert.match(main, /const terminalTurnPayload = settleStoppedRealtimeTurn\(realtimeClient/);
  assert.match(main, /terminalTurnPayload \|\| \{ phase: "done", mode: "chat"/);
  assert.match(remote, /setRemoteLiveOutputSuppressed\(Boolean\(params\.suppressed\) && method !== "thread\/realtime\/transcript\/done"\)/);
