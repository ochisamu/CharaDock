...
test("Realtime sessions start only from voice input, accept typed turns, and keep transcript deltas intact", () => {
  ...
  assert.match(control, /await api\.appendCodexRealtimeText\(message, selectedSkillIds\)/);
  ...
  assert.match(control, /streamingMessage && !streamingMessage\.dataset\.workRunId[\s\S]*finishDetachedRealtimeWork\(\)/);
